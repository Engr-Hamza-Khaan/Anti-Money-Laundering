...
function badgeArt(kind){
  return `<img src="assets/badge-${kind}.png" alt="" width="64" height="64">`;
}
...
function confetti(){
  ...
  for (let i=0;i<130;i++) ps.push({
    x: innerWidth/2 + (Math.random()-.5)*260, y: innerHeight*0.34,
...
function applyLang(){
  ...
  const splashLangLbl = document.getElementById('splashlanglbl');
...
function setLang(lang, fromSplash){
  ...
  if (skip) skip.click();
  else if (!document.getElementById('splash')) home();
}
...
function startVoiceEl(file){
  const a = new Audio(VOICE_BASE + vf(file));
  _voice = a;
  attachVoiceEvents(a);
  const p = a.play();
...
function renderRank(){
  ...
  document.getElementById('rankchip').innerHTML =
    rankBadge(r.stars) +
...
function closeModal(){
  const el = document.getElementById('cmodal');
  if (el) el.remove();
...
/* =====================================================================
   ...
   ===================================================================== */
const ART = {
  'elena-explain':'assets/elena-explain.png?v=3',
  'elena-concern':'assets/elena-concern.png?v=3',
...
function results(o){
  ...
  `);
  bindBack();
  document.getElementById('againbtn').onclick = o.again;
  ...
  function draw(){
    ...
    function chooseAction(a, btn){
      ...
      if ((a === 'flag' || a === 'escalate') && c.reasons && !stage2){
        ...
        document.getElementById('reasons').classList.add('show');
        beep('pick');
  ...
  stage.querySelectorAll('.vcard').forEach(el=>{
    el.onclick = ()=>{
      ...
      if (!S.seen.includes(g.k)){
        ...
        document.getElementById('seencount').textContent = S.seen.length + '/' + GLOSSARY.length;
...
function screenCertificate(avg){
  ...
  document.getElementById('homebtn2').onclick = home;
  if (passed) confetti();
}
...
function splash(){
  return new Promise(resolve=>{
    ...
    const langBox = document.getElementById('splashlang');
    if (langBox){
...
export function boot(){
  ...
  startAmbient();
  document.getElementById('brand').onclick = home;
  ...
  document.getElementById('langbtn').onclick = ()=> setLang(isUr() ? 'en' : 'ur');
